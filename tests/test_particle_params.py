...
def _session(systems):
    s = AppSession()
    s.particles.pcf = _StubService(systems)
    return s
...
class ParamsListTests(unittest.TestCase):
    ...
    def test_every_param_of_the_schema_is_reported(self):
        ...
        self.assertEqual(len(self.s.particles.particle_params('sys')), len(SIMPLE_PARAMS))

    def test_value_comes_from_the_module(self):
        drag = next(p for p in self.s.particles.particle_params('sys') if p['key'] == 'drag')
    ...
    def test_missing_module_gives_none_and_a_placeholder(self):
        ...
        life = next(p for p in self.s.particles.particle_params('sys')
                    if p['key'] == 'lifetime')
    ...
    def test_unknown_system_is_empty_not_an_error(self):
        self.assertEqual(self.s.particles.particle_params('нет такой'), [])
...
class SetParamTests(unittest.TestCase):
    ...
    def test_write_reaches_the_service_and_returns_systems(self):
        res = self.s.particles.set_particle_param('sys', 'drag', 0.5)
        self.assertIn('systems', res)
        self.assertEqual(self.s.particles.pcf.writes,
                         [('sys', 'operators', 0, 'drag', 0.5)])

    def test_missing_module_is_created_when_the_schema_allows(self):
        self.s.particles.set_particle_param('sys', 'lifetime', [1.0, 2.0])
        self.assertEqual(self.s.particles.pcf.added,
    ...
    def test_emitter_module_is_never_created(self):
        ...
        res = self.s.particles.set_particle_param('sys', 'spawn_burst', 50)
        self.assertIn('error', res)
        self.assertEqual(self.s.particles.pcf.added, [])

    def test_unknown_param_and_system(self):
        self.assertIn('error', self.s.particles.set_particle_param('sys', 'нет', 1))
        self.assertIn('error', self.s.particles.set_particle_param('нет', 'drag', 1))

    def test_without_a_loaded_pcf(self):
        self.assertIn('error', AppSession().particles.set_particle_param('sys', 'drag', 1))
...
class ExpertModeTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        mod = self.s.particles.pcf.systems_json()['sys']['operators'][0]
        mod['attrs']['functionname'] = {'t': 'string', 'v': 'Movement Basic'}
        mod['attrs']['id'] = {'t': 'string', 'v': 'x'}

    def _groups(self):
        return {g['group']: g for g in self.s.particles.particle_system('sys')['groups']}

    def test_system_attrs_come_first_as_their_own_block(self):
        groups = self.s.particles.particle_system('sys')['groups']
    ...
    def test_attrs_are_sorted(self):
        self.s.particles.pcf.systems_json()['sys']['operators'][0]['attrs'].update(
            {'a_last': {'t': 'float', 'v': 1.0}, 'a_first': {'t': 'float', 'v': 2.0}})
        names = [a['name'] for a in self._groups()['operators']['modules'][0]['attrs']]
        self.assertEqual(names, sorted(names))

    def test_unknown_system_is_empty(self):
        self.assertEqual(self.s.particles.particle_system('нет'), {})

    def test_edit_writes_through_the_service(self):
        res = self.s.particles.set_particle_attr('sys', 'operators', 0, 'drag', 0.9)
        self.assertIn('systems', res)
        self.assertEqual(
            self.s.particles.pcf.systems_json()['sys']['operators'][0]['attrs']['drag']['v'],
            0.9)

    def test_edit_of_a_missing_attr_is_an_error_not_a_silent_pass(self):
        res = self.s.particles.set_particle_attr('sys', 'operators', 0, 'нет такого', 1)
        self.assertIn('error', res)
...
class StructureTests(unittest.TestCase):
    ...
    def setUp(self):
        self.s = _session({'sys': _system(
            operators=[_mod('Movement Basic', {'drag': 0.25})]),
            'other': _system()})
        self.s.particles.pcf._systems['sys']['children'] = [
    ...
    def test_every_change_returns_systems_and_tree(self):
        ...
        res = self.s.particles.add_particle_module('sys', 'forces', 'random force')
        self.assertIn('systems', res)
        self.assertIn('tree', res)
        self.assertTrue(all('kids' in n for n in res['tree']))

    def test_remove_module(self):
        self.s.particles.remove_particle_module('sys', 'operators', 0)
        self.assertEqual(self.s.particles.pcf.systems_json()['sys']['operators'], [])

    def test_remove_module_out_of_range_is_an_error(self):
        self.assertIn('error', self.s.particles.remove_particle_module('sys', 'operators', 7))

    def test_children_are_listed_with_their_index(self):
        """Отцепляют по индексу: одна и та же система может висеть дважды."""
        self.assertEqual(self.s.particles.particle_children('sys'),
                         [{'index': 0, 'name': 'other', 'delay': 0.0}])

    def test_children_of_unknown_system(self):
        self.assertEqual(self.s.particles.particle_children('нет'), [])

    def test_without_a_loaded_pcf_every_action_says_so(self):
        empty = AppSession()
        for call in (lambda: empty.particles.add_particle_module('s', 'forces', 'f'),
                     lambda: empty.particles.remove_particle_module('s', 'forces', 0),
                     lambda: empty.particles.remove_particle_attr('s', None, 0, 'a'),
                     lambda: empty.particles.duplicate_particle_system('s', 'x'),
                     lambda: empty.particles.add_particle_layer('s')):
...
class CopyPayloadTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.s.particles.pcf.systems_json()['sys']['attrs']['radius'] = {
            't': 'float', 'v': 5.0}
    ...
    def test_whole_system_is_marked_full(self):
        ...
        payload = self.s.particles.copy_particle_params('sys')['payload']
    ...
    def test_single_module_is_not_full(self):
        payload = self.s.particles.copy_particle_params('sys', 'operators', 0)['payload']
    ...
    def test_single_attribute_of_a_module(self):
        ...
        payload = self.s.particles.copy_particle_params(
    ...
    def test_single_attribute_of_the_system(self):
        payload = self.s.particles.copy_particle_params('sys', None, None, 'radius')['payload']
        self.assertEqual(payload['attrs'], {'radius': {'t': 'float', 'v': 5.0}})
        self.assertEqual(payload['modules'], {})

    def test_whole_group(self):
        self.s.particles.pcf.systems_json()['sys']['operators'].append(
            _mod('Radius Scale', {'radius_start_scale': 1.0}))
        payload = self.s.particles.copy_particle_params('sys', 'operators')['payload']
    ...
    def test_unknown_system_and_module(self):
        self.assertIn('error', self.s.particles.copy_particle_params('нет'))
        self.assertIn('error', self.s.particles.copy_particle_params('sys', 'operators', 9))
        self.assertIn('error',
                      self.s.particles.copy_particle_params('sys', 'operators', 0, 'нет'))
        self.assertIn('error', self.s.particles.copy_particle_params('sys', 'forces'))
...
class HistoryTests(unittest.TestCase):
    ...
    def setUp(self):
        self.s = AppSession()
        self.s.particles.pcf = _HistoryService({'sys': _system(
            operators=[_mod('Movement Basic', {'drag': 0.25})])})
        self.s.particles._history_reset()

    def _drag(self):
        return (self.s.particles.pcf.systems_json()['sys']['operators'][0]
                ['attrs']['drag']['v'])

    def test_after_load_there_is_nothing_to_undo(self):
        self.assertEqual(self.s.particles.particle_history(),
                         {'undo': False, 'redo': False})

    def test_edit_then_undo_and_redo(self):
        self.s.particles.set_particle_attr('sys', 'operators', 0, 'drag', 0.9)
        self.assertEqual(self._drag(), 0.9)
        self.assertTrue(self.s.particles.particle_history()['undo'])

        self.s.particles.undo_particles(-1)
        self.assertEqual(self._drag(), 0.25)
        self.assertTrue(self.s.particles.particle_history()['redo'])
    ...
    def test_a_new_edit_drops_the_redo_branch(self):
        self.s.particles.set_particle_attr('sys', 'operators', 0, 'drag', 0.9)
        self.s.particles.undo_particles(-1)
        self.s.particles.set_particle_attr('sys', 'operators', 0, 'drag', 0.5)
        self.assertFalse(self.s.particles.particle_history()['redo'])

    def test_undo_itself_is_not_an_edit(self):
        """Иначе откат добавлял бы снимок и возврат стал бы недостижим."""
        self.s.particles.set_particle_attr('sys', 'operators', 0, 'drag', 0.9)
        before = len(self.s.particles._particle_history)
        self.s.particles.undo_particles(-1)
        self.assertEqual(len(self.s.particles._particle_history), before)

    def test_beyond_the_edge_is_an_error(self):
        self.assertIn('error', self.s.particles.undo_particles(-1))
        self.assertIn('error', self.s.particles.undo_particles(1))

    def test_history_is_capped(self):
        """Снимок — копия всего дерева; помним ограниченное число шагов."""
        for i in range(self.s.particles.HISTORY_LIMIT + 10):
            self.s.particles.set_particle_attr('sys', 'operators', 0, 'drag', i / 100)
        self.assertEqual(len(self.s.particles._particle_history), self.s.particles.HISTORY_LIMIT)

    def test_without_a_loaded_pcf(self):
        self.assertIn('error', AppSession().particles.undo_particles(-1))
...
class ControlPointTests(unittest.TestCase):
    ...
    def test_points_of_the_effect_and_its_children(self):
        ...
        systems = s.particles.pcf.systems_json()
        ...
        used = s.particles.particle_control_points('root')['used']
        self.assertIn(9, used)
        self.assertIn(3, used)

    def test_unknown_system_and_no_pcf(self):
        s = _session({'sys': _system()})
        self.assertEqual(s.particles.particle_control_points('нет')['used'], [])
        self.assertEqual(AppSession().particles.particle_control_points('sys')['used'], [])
...
class MaterialTests(unittest.TestCase):
    ...
    def setUp(self):
        self.s = AppSession()
        self.s.particles.pcf = _MaterialService(
            {'sys': _system()},
    ...
    def test_only_the_materials_of_the_chosen_effect(self):
        ...
        names = [c['name'] for c in self.s.particles.particle_materials('root')]
        self.assertEqual(names, ['effects/glow.vmt', 'particle/smoke.vmt'])
        # Без системы — весь файл: так материалы смотрят до выбора эффекта.
        self.assertEqual(len(self.s.particles.particle_materials()), 3)
    ...
    def test_shared_child_is_not_listed_twice(self):
        ...
        svc._systems['kid']['attrs']['material'] = {
            't': 'string', 'v': 'effects/glow.vmt'}
        self.assertEqual([c['name'] for c in self.s.particles.particle_materials('root')],
                         ['effects/glow.vmt'])
    ...
    def test_cycle_in_children_does_not_hang(self):
        ...
        self.assertEqual(self.s.particles.particle_materials('a'), [])

    def test_cards_carry_size_sheet_and_custom_flag(self):
        cards = {c['name']: c for c in self.s.particles.particle_materials()}
    ...
    def test_material_without_a_picture_still_gets_a_card(self):
        ...
        self.s.particles.pcf._materials['effects/none.vmt'] = {}
        cards = {c['name']: c for c in self.s.particles.particle_materials()}
        self.assertEqual(cards['effects/none.vmt']['dataUrl'], '')

    def test_reset_of_a_game_texture_is_an_error(self):
        self.assertIn('error',
                      self.s.particles.reset_particle_texture('particle/smoke.vmt'))
        self.assertIn('systems',
                      self.s.particles.reset_particle_texture('effects/glow.vmt'))

    def test_natural_colors_report_how_many_modules_went(self):
        res = self.s.particles.use_particle_texture_colors('sys')
        self.assertEqual(res['removed'], 3)
        self.assertIn('materials', res)

    def test_without_a_loaded_pcf(self):
        empty = AppSession()
        self.assertEqual(empty.particles.particle_materials(), [])
        self.assertIn('error', empty.particles.reset_particle_texture('x'))
        self.assertIn('error', empty.particles.use_particle_texture_colors('sys'))
