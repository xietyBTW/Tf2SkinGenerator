"""
Сборщик VPK-модов для текстур тел персонажей TF2.

Логика такая же, как для рук:
...
Для персонажей есть основная текстура (RED) и дополнительная (BLU):
  - Первая текстура берётся из основного image_path
  - BLU текстура запрашивается через extra_texture_callback
  - Если пользователь отказался — копируем RED текстуру
"""
...
from src.data.player_characters import PLAYER_CHARACTERS, get_player_body_textures
...
class PlayerSkinBuildService:
    """Сборка VPK-модов для текстур тел персонажей."""
    ...
    def build(
        ...
        """
        Создаёт VTF файлы для тела персонажа и помещает их в vpkroot.

        Args:
            ctx:                    Контекст сборки (temp директории, vpkroot и т.д.)
            mode:                   Ключ режима (например, "scout_body")
            image_path:             Путь к исходному изображению (основная RED текстура)
        ...
        try:
            textures = get_player_body_textures(mode)
            if not textures:
                return False, f"Unknown player body mode: {mode}"
            ...
            for idx, (folder, vtf_name) in enumerate(textures):
                ...
                logger.info(f"[player_skin] Обрабатываем текстуру {idx + 1}/{n_textures}: {vtf_name}")
                ...
                if idx == 0:
                    ...
                    if custom_vtf_path:
                        ...
                        logger.info(f"[player_skin] Скопирован custom VTF: {dest_vtf}")
                    else:
                        PlayerSkinBuildService._convert_image_to_vtf(
                            image_path, dest_vtf, vtf_output_dir,
                            size, format_type, vtf_flags, merged_options
                        )
                        logger.info(f"[player_skin] Создан VTF: {dest_vtf}")
                    ...
                    if extra_texture_callback:
                        ...
                        if extra_src and not Path(extra_src).is_file():
                            logger.warning(f"[player_skin] Файл не найден: {extra_src}")
                            extra_src = None
                    ...
                    if extra_src:
                        ...
                        logger.info(f"[player_skin] Доп. текстура из пользователя: {dest_vtf}")
                    else:
                        ...
                        if primary_vtf_path and primary_vtf_path.exists():
                            ...
                            logger.info(
                                f"[player_skin] Доп. текстура скопирована из первичной: "
                                f"{primary_vtf_path.name} → {dest_vtf.name}"
                            )
                        else:
                            logger.warning(f"[player_skin] Первичная текстура недоступна для копирования: {dest_vtf}")
            ...
            error_msg = str(e)
            logger.error(f"[player_skin] Ошибка при обработке текстур персонажа: {error_msg}", exc_info=True)
