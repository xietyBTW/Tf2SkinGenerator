...
Руки берут позу из модели анимаций, а оружие — ведомое: КАЖДАЯ его кость,
одноимённая с костью анимации, получает от неё мировую матрицу
(`CBoneMergeCache::UpdateCache`, source-sdk-2013). `$bonemerge` в QC на отбор
не влияет — это только флаг производительности. Так анимация класса и водит
подвижными частями (барабан, крышка), и так же ПРЯЧЕТ их: патроны Shortstop и
револьвера в idle унесены на 60–110 единиц за кадр и возвращаются к оружию
только в перезарядке.
...
def bonemerge_skinning(weapon_ref_smd: str, pose: Rig,
    ...
    Список из QC здесь — только для статичной позы: в bind-позе рук
    `weapon_bone_1..4` выстроены лесенкой точек крепления, и слияние с ними
    разносило револьвер на 73 единицы вместо его восемнадцати.

    Списка нет — сливаем все одноимённые кости, как движок. Через них анимация
    класса и водит подвижными частями: крышку флергана открывает
    `weapon_bone_2` из `fg_fire`, ведь своих кадров у моделей оружия нет.
...
def merged_bone_names(weapon_ref_smd: str, poses,
    ...
    Кость, которая в кадре улетает далеко от оружия, — не чужая: так анимация
    прячет патрон (у Ответного удара в `reload_loop` он уходит за кадр). Её
    сливаем, как и движок.

    Args:
        poses: один Rig или несколько — кость проходит, если есть в каждом.
    """
    bind = load_rig(weapon_ref_smd)
    if bind is None:
        return []
...
def _merge_targets(bind: Rig, pose: Rig,
    ...
    if merge_bones:
        ...
        if wanted:
            return wanted
        logger.info(
            "[vm] $bonemerge не пересёкся со скелетом руки — берём все общие")
    # Как движок: все одноимённые кости. Кроме служебного корня НАД хватом —
    # мировой ноль, слияние с которым вешало сапёр в начале координат.
    above = _above_anchor(bind) if ANCHOR_BONE in available else set()
    return available - above
