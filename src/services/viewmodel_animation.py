...
def _weapon_part(weapon_ref_smd: str, bind: viewmodel_pose.Rig,
    ...
    Своего скелета у него в сцене нет: каждая вершина принадлежит тем костям
    руки, которые ведут её кости оружия, — с теми же долями, что в SMD.
    ...
    def links_of(vert: dict) -> List[Tuple[int, float]]:
        """Веса вершины, переведённые с костей оружия на кости руки.

        Раньше здесь бралась ОДНА кость — первая в списке, весом 1. Для
        обычного оружия это то же самое: в руку садится единственная кость, и
        все вершины ведёт она — доли просто складывались в единицу.

        Но бывают модели, где в руку садится несколько костей и вершина
        принадлежит им долями. У Мутировавшего молока таких вершин 8902 из
        15906 (55%) на девяти костях: банка, молоко и монстр внутри связаны
        мягко. Геометрия запекалась по ВСЕМ весам, а анимация вела каждую
        вершину одной костью — меш рвало по швам, и молоко уезжало из стекла.

        Несколько костей оружия могут вести одну и ту же кость руки (курок и
        корпус оба едут за `weapon_bone`) — их доли складываются, иначе
        последняя затирала бы предыдущие. Нормировку и отбор четырёх сильнейших
        делает `_weights`.
        """
        out: Dict[int, float] = {}
        for bone, weight in vert.get("links") or ():
            slot = index_of.get(drivers.get(bone, ""))
            if slot is None or weight <= 0.0:
                continue
            out[slot] = out.get(slot, 0.0) + float(weight)
