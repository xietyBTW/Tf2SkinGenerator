...
Доставка. Qt при AutoConnection сам перекидывал слот в поток получателя;
здесь слот вызывается прямо в потоке воркера, если никто не поставил
диспетчер (`set_dispatcher`). Странице в WebView2 это подходит: она получает
события очередью и разбирает их у себя, а не трогает виджеты.
"""
