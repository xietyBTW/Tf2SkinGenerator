...
# Копируем иконку в выходную папку (для использования в runtime)
if (Test-Path $IconPath) {
    Write-Host "`n[3.6/4] Copying icon to output..." -ForegroundColor Cyan
    $IconDest = Join-Path $OutputDir "icon.ico"
    Copy-Item -Path $IconPath -Destination $IconDest -Force
    Write-Host "Icon copied successfully" -ForegroundColor Gray
}
