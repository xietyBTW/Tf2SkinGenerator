...
A Windows desktop app for building **Team Fortress 2 VPK skin mods** without touching the
command line. Pick a weapon (or hat, character part, skybox…), drop in your texture, and get a
ready‑to‑install `.vpk` — the app handles VTF conversion, VMT generation, model
decompile/recompile and packaging for you.
...
- **Weapon reskins** — replace the texture of any stock weapon.
- **Custom models** — swap the model itself (decompile → recompile via Crowbar + `studiomdl`).
- **Hats** — per‑class and per‑style hat mods, with optional game‑paint support.
...
- **Skyboxes** — replace the sky from a panorama or six face textures, with 3D preview.
- **Projectiles, pickups, taunt props.**
- **Live 2D + 3D preview** before you build.
- **Diagnostics tab** — inspect an existing VPK mod and flag common breakage.
- English / Russian UI.
...
Needs a couple of third‑party tools that aren't committed — see
[docs/en/building-from-source.md](docs/en/building-from-source.md).
