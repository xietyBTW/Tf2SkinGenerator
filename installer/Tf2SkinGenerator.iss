...
#define AppExe "Tf2SkinGenerator.exe"
...
Name: "removedata"; Description: "Удалить пользовательские данные (config, export) при удалении"; GroupDescription: "Удаление"; Flags: unchecked
  ...
  begin
    ...
    begin
      ...
      try
        DownloadPage.Show;
        try
          DownloadOk := DownloadPage.Download;
        finally
          DownloadPage.Hide;
        end;
      except
      ...
      if Attempt < 3 then
      begin
        if MsgBox('Ошибка при скачивании файла. Повторить попытку?', mbError, MB_RETRYCANCEL) = IDCANCEL then
          break;
      end;
    end;
...
Type: filesandordirs; Name: "{app}\config"; Tasks: removedata
Type: filesandordirs; Name: "{app}\export"; Tasks: removedata
