...
; Bootstrap installer that downloads from specific release tag "App"
...
; Используем конкретный тег "App" вместо "latest"
#define DownloadURL "https://github.com/{#RepoOwner}/{#RepoName}/releases/download/App/{#AssetName}"
  ...
  begin
    ...
    try
      ...
      DownloadArgs := Format('-NoProfile -ExecutionPolicy Bypass -Command "try { $ProgressPreference = ''SilentlyContinue''; Invoke-WebRequest -Uri ''%s'' -OutFile ''%s'' -UseBasicParsing; exit 0 } catch { Write-Host $_.Exception.Message; exit 1 }"', [
        'https://github.com/{#RepoOwner}/{#RepoName}/releases/download/App/{#AssetName}',
        ZipPath
      ]);
      
      Log('Downloading archive with PowerShell...');
      Log('URL: https://github.com/{#RepoOwner}/{#RepoName}/releases/download/App/{#AssetName}');
      Log('Save to: ' + ZipPath);
      ...
      UnzipArgs := Format('-NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference = ''Stop''; $tmpDir = Join-Path $env:TEMP ''Tf2SkinGenerator-install''; if (Test-Path $tmpDir) { Remove-Item -Recurse -Force $tmpDir }; Expand-Archive -Path ''%s'' -DestinationPath $tmpDir -Force; $sourceDir = Join-Path $tmpDir ''Tf2SkinGenerator''; if (Test-Path $sourceDir) { $destDir = ''%s''; New-Item -ItemType Directory -Path $destDir -Force | Out-Null; Get-ChildItem -Path $sourceDir -Recurse | Copy-Item -Destination { Join-Path $destDir $_.FullName.Substring($sourceDir.Length + 1) } -Force; Remove-Item -Recurse -Force $tmpDir } else { throw ''Source directory Tf2SkinGenerator not found after extraction'' }"', [
        ZipPath,
        ExpandConstant('{app}')
      ]);
      ...
      begin
        DownloadProgressPage.Hide;
        MsgBox(Format('Ошибка: после распаковки не найден файл %s'#13#10'Возможно, файлы не были правильно распакованы.', [ExpandConstant('{app}\{#AppName}.exe')]), mbError, MB_OK);
        Result := False;
        Exit;
      end;
